...
void Rasterizer::Rasterize(const Vertex &a, const Vertex &b, vector<Fragment> &res){
    if (anti_aliasing_depth){
        ...
        Vertex aa = a; aa.x *= scale, aa.y *= scale, aa.gray *= scale;
        Vertex bb = b; bb.x *= scale, bb.y *= scale, aa.gray *= scale;
        ...
        for (vector<Fragment>::iterator it = temp.begin(); it != temp.end(); it++){
            int x = it->x / scale;
            int y = it->y / scale;
            if (x >= 0 && x < width && y >= 0 && y < height) g[y][x] += it->gray, z[y][x] += it->z;
        }
        for (int i = 0; i < height; i++){
            for (int j = 0; j < width; j++){
                if (g[i][j] > EPS){
                    res.push_back(Fragment(j, i, z[i][j] / size, g[i][j] / size));
                }
            }
        }
...
void Rasterizer::Rasterize(const Vertex &p0, const Vertex &p1, const Vertex &p2, vector<Fragment> &res){
    ...
    if (r_mode == FILLING){
        for (int i = 0, j, k; i < height; i++){
            for (j = 0; j < width; j++) if (ch[i][j]) break;
            for (k = width - 1; k >= 0; k--) if (ch[i][k]) break;
            for (; j <= k; j++){
                if (!ch[i][j]) ch[i][j] = 1 << 3;
            }
        }
    }
